...
test "compression collisions survive dirty reuse and an encoding failure" {
    ...
    try std.testing.expectEqual(bucket, std.hash.Wyhash.hash(0, names[1].wire()) % 16384);
    var encoder: wire.Encoder = undefined;
    var output: [512]u8 = undefined;
    for (0..3) |_| {
        try encoder.init(&output, &.{});
        try std.testing.expect(!encoder.occupied.isSet(bucket));
        ...
        for (0..4) |index| {
            ...
            try std.testing.expectEqualSlices(u8, names[index % 2].wire(), decoded.wire());
        }
        ...
        try std.testing.expectError(error.NoSpace, encoder.question(&names[0], 1, 1));
        try std.testing.expect(encoder.occupied.isSet(bucket));
        ...
        try std.testing.expectEqualSlices(u8, names[1].wire(), decoded.wire());
    }
}
