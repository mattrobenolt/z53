...
const Pools = blk: {
    ...
    for (sizes, &types) |size, *kind| kind.* = heap.MemoryPool([size]u8);
    break :blk std.meta.Tuple(&types);
};

pub const Storage = struct {
    fixed: heap.FixedBufferAllocator = .init(&.{}),
    pools: Pools = undefined,
    live_bytes: u32 = 0,
    live_class_bytes: u32 = 0,

    pub fn init(self: *Storage, allocator: Allocator, limit: u32) error{OutOfMemory}!void {
    ...
    pub fn destroy(self: *Storage, bytes: []u8, selected: u8) void {
        assert(selected == class(bytes.len));
        inline for (&self.pools, 0..) |*pool, index| {
            if (selected == index) {
